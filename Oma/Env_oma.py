...
class Environment:
    def __init__(self, k, N, noise_power, state_dim, action_dim):
        self.k = k
        self.C = N  # number of clusters/beams
        self.N = N
        self.B = 1 * k  # Mhz
        self.T = 50
        ...
        self.P_k_list = np.random.normal(scale=10, size=(3, self.k))
        self.P_k_list[2, :] = 0
        ...
        self.w_ak_list = np.zeros(shape=(self.N, self.N)) + 0 * 1j
        self.h_beam_list = np.zeros(shape=(self.N, self.N)) + 0 * 1j
        self.ee_list = []
        self.ee_sum = 0
        self.sum_rate = 0
        self.sum_sinr = 0
    ...
    def calculate_loss(self):
        for k in range(self.k):
            ...
            los_ak = self.los_ak
            r_ak = np.sqrt(PL_ak) * (np.sqrt(self.Rice / (1 + self.Rice)) * los_ak + np.sqrt(1 / (1 + self.Rice)) * Nlos_ak)
            ## print("user: ", k, "r_ak: ", r_ak)
            self.r_ak_list[k] = r_ak
        # print("r_ak_list： ", self.r_ak_list)

        #ZF
        self.h_beam_list = self.r_ak_list[0:self.C, :]
        HH = self.h_beam_list
        H = self.h_beam_list.conj().T
        W = np.dot(H, np.linalg.inv(np.dot(HH, H))) # W in paper

        # H = self.h_beam_list
        # HH = self.h_beam_list.conj()
        # W = np.dot (H, np.linalg.inv(np.dot(HH, H))) # W in paper
        # check = np.dot(HH, W)
        # self.w_ak_list = W
        # print("W: ", self.w_ak_list)
        # print("W*H: ", check)
        # self.t += 1

        W_real_list = [0] * (self.N*self.N)
        W_imag_list = [0] * (self.N*self.N)
        # W_reconstruct = [0] * (self.N*self.N)
        W_sum = 0

        W_list = np.reshape(W, [1, self.N*self.N])
        for i in range(self.N*self.N):
            W_real_list[i] = W_list[0,i].real
            W_imag_list[i] = W_list[0,i].imag

        # 归一化
        for i in range(self.N*self.N):
            W_sum += W_real_list[i]**2 + W_imag_list[i]**2
        W_sum = math.sqrt(W_sum)
        W = W/W_sum

        self.w_ak_list = W

            # W_real_list[i] = W_real_list[i]/(sum(W_real_list)+sum(W_imag_list))
            # W_imag_list[i] = W_imag_list[i]/(sum(W_real_list)+sum(W_imag_list))
            # W_reconstruct[i] = W_real_list[i] + 1j*W_real_list[i]

        # self.w_ak_list = np.reshape(W_reconstruct, [self.N, self.N])
    ...
    def calculate_data_rate(self):
        ...
        # beamforming
        for k in range(self.k):
            cluster = self.clustering_result[k]
            g_k_H = abs(np.dot(self.r_ak_list[k], self.w_ak_list[cluster])) ** 2
            self.h_list[k] = g_k_H
            ...
            for j in range(self.k):
                if self.clustering_result[i] == self.clustering_result[j]:  # in the same cluster
                    if i != j and self.h_list[j] > self.h_list[i]:
                        # print("i: ", i, "j: ", j, "power_j: ", self.power_allocation_list[j], "h_i: ", self.h_list[i])
    ...
    def step(self, action):
        ...
        # # alpha list
        # alpha_list = [0] * self.k
        # original_alpha = action[self.k:self.k + self.k]
        ...
        # beta
        self.beta = action[self.k + self.k]
